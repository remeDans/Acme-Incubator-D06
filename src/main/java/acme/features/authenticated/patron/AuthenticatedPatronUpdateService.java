...
public class AuthenticatedPatronUpdateService implements AbstractUpdateService<Authenticated, Patron> {
	...
	@Autowired
	private PatronCreditCardRepository		creditCardRepository;
	...
	public void unbind(final Request<Patron> request, final Patron entity, final Model model) {
		...
		Integer patronId = entity.getId();

		model.setAttribute("hasCreditcardPatron", false);

		for (CreditCard c : this.creditCardRepository.findManyAll()) {
			if (c.getPatron() != null) {
				Integer patronIdCreditCard = c.getPatron().getId();
				if (patronId.equals(patronIdCreditCard)) {
					model.setAttribute("hasCreditcardPatron", true);
					break;
				}
			}
		}
